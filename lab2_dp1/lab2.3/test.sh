#!/bin/bash
# Script for testing socket assignments 2.3 and 3.1
...
if [ "$SERVER2_IS_PRESENT" = true ] ; then
    echo -e "\n\n*** PART 4: TESTS ON THE STUDENT'S SERVER2 WITH A REFERENCE CLIENT *************"
    test_suite=4
    test_number=0
    client_run=0
	...
	echo -e "---Skipping test of Part 4 because the student's SERVER2 didn't compile---"
    fi
    echo "*** END OF TESTING PART 4 *************"
fi

#********************************** TEST SUITE 5 ********************************************************
if [ "$SERVER2_IS_PRESENT" = true ] ; then
    echo -e "\n\n*** PART 5: TESTS ON THE STUDENT'S SERVER2 WITH THE STUDENT'S CLIENT *************"
    test_suite=5
    test_number=0
    client_run=0

    if [[ $TEST_03_PASSED == true ]] && [[ $TEST_02_PASSED == true ]] ; then

        testClientServerInteraction "$SERVER2" "$TESTCLIENT1"
        killProcesses
    else
	echo -e "---Skipping test of Part 5 because the student's SERVER2 didn't compile---"
    fi
    echo "*** END OF TESTING PART 5 *************"
fi
